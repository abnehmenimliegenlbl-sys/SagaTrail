/**
 * Generated by orval v8.18.0 🍺
 * Do not edit manually.
 * Api
 * API specification
 * OpenAPI spec version: 0.1.0
 */

export * from './achievement';
export * from './aerialway';
export * from './analyzeObjectRequest';
export * from './analyzeObjectRequestMediaType';
export * from './analyzeObjectResponse';
export * from './avalancheBulletin';
export * from './avalancheBulletinReason';
export * from './cancelMeetupRequest';
export * from './catalogCanton';
export * from './catalogCoordinates';
export * from './catalogResponse';
export * from './catalogRoute';
export * from './catalogRouteSeason';
export * from './catalogSaga';
export * from './catalogSagaKoordinatenSicherheit';
export * from './catalogSagaSummaries';
export * from './claimKantonspackBody';
export * from './claimKantonspackResponse';
export * from './claimPackRewardBody';
export * from './claimReferralCode200';
export * from './claimReferralCodeBody';
export * from './communityClaimInput';
export * from './communityInput';
export * from './communityInvitation';
export * from './communityMembership';
export * from './communitySummary';
export * from './communityUpdate';
export * from './createMeetupRequest';
export * from './createMeetupRequestPace';
export * from './createSafetyShareRequest';
export * from './customDrawnRouteBody';
export * from './customRouteWaypoint';
export * from './customWaypointsBody';
export * from './deleteMeetupPhoto200';
export * from './errorResponse';
export * from './geocodePlace';
export * from './getAerialwaysParams';
export * from './getAvalancheBulletinLang';
export * from './getAvalancheBulletinParams';
export * from './getCantonRoutesParams';
export * from './getCustomRouteParams';
export * from './getMeetupPhotos200';
export * from './getMeetupsParams';
export * from './getMyReferralCode200';
export * from './getPartnersParams';
export * from './getPeakPoisParams';
export * from './getPoiDetailParams';
export * from './getPoisParams';
export * from './getPoiStoryParams';
export * from './getRoutePhotoParams';
export * from './getRouteSurfacesParams';
export * from './getSagaPhotoParams';
export * from './getTransportNearbyParams';
export * from './getTransportStationboardParams';
export * from './getWeatherParams';
export * from './gpxImportBody';
export * from './healthStatus';
export * from './hikeSession';
export * from './localizedSummary';
export * from './meetup';
export * from './meetupActionResponse';
export * from './meetupAttendanceRequest';
export * from './meetupAttendanceRequestStatus';
export * from './meetupAttendanceResponse';
export * from './meetupAttendanceResponseAttendanceStatus';
export * from './meetupDetail';
export * from './meetupJoinResponse';
export * from './meetupLifecycleResponse';
export * from './meetupLifecycleResponseStatus';
export * from './meetupListResponse';
export * from './meetupMessage';
export * from './meetupMessageRequest';
export * from './meetupMessageResponse';
export * from './meetupParticipant';
export * from './meetupParticipantAttendanceStatus';
export * from './meetupParticipantGroupAchievementsItem';
export * from './meetupReportRequest';
export * from './meetupReportRequestReason';
export * from './meetupShareResponse';
export * from './meetupStatus';
export * from './narrationInput';
export * from './narrationInputProvider';
export * from './objectRecognitionCandidate';
export * from './objectRecognitionLimitError';
export * from './objectRecognitionLimitErrorCode';
export * from './partner';
export * from './partnerKategorie';
export * from './partnerPaket';
export * from './poi';
export * from './poiDetailResponse';
export * from './poiStory';
export * from './premiumUpdate';
export * from './prepareMeetupPhotoShare200';
export * from './prepareMeetupPhotoShareBody';
export * from './profile';
export * from './profileAgeTier';
export * from './profileArchetype';
export * from './profileInput';
export * from './profileInputAgeTier';
export * from './profileInputArchetype';
export * from './progressSyncInput';
export * from './progressSyncResponse';
export * from './routeDataSource';
export * from './routeDataSources';
export * from './routePhoto';
export * from './routeSurfacePoint';
export * from './routeSurfacesResponse';
export * from './safetyShare';
export * from './safetyShareLocation';
export * from './safetySharePublicStatus';
export * from './safetySharePublicStatusLatestLocation';
export * from './safetySharePublicStatusStatus';
export * from './sagaQuelle';
export * from './searchPlacesParams';
export * from './sharedMeetup';
export * from './sharedMeetupStatus';
export * from './storyChapter';
export * from './storyDecision';
export * from './storyDecisionOption';
export * from './storyRequest';
export * from './storyRequestAgeTier';
export * from './storyRequestArchetype';
export * from './storyResponse';
export * from './terrainAreaRequest';
export * from './terrainAreaRequestOptions';
export * from './terrainAreaResponse';
export * from './terrainAreaResponseBounds';
export * from './terrainAreaResponseOrigin';
export * from './terrainCorridorCell';
export * from './terrainCorridorRequest';
export * from './terrainCorridorRequestOptions';
export * from './terrainCorridorResponse';
export * from './terrainCorridorResponseBounds';
export * from './terrainCorridorResponseOrigin';
export * from './trailConditionInput';
export * from './trailConditionInputCondition';
export * from './trailConditionReport';
export * from './trailConditionReportCondition';
export * from './transportDeparture';
export * from './transportNearbyResponse';
export * from './transportNearbyResponseStation';
export * from './transportStationboard';
export * from './transportStationboardStation';
export * from './updateMeetupPhotoConsent200';
export * from './updateMeetupPhotoConsentBody';
export * from './updateMeetupRequest';
export * from './updateMeetupRequestPace';
export * from './uploadMeetupPhoto201';
export * from './uploadMeetupPhotoDepictedPeopleConsent';
export * from './uploadMeetupPhotoParams';
export * from './uploadMeetupPhotoRightsConsent';
export * from './weatherReport';
export * from './wikiSummary';
