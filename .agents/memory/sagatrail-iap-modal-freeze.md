...
**Expo Go caveat:** in Expo Go, `react-native-purchases` has no native module and silently falls back to "Browser Mode" (`purchases-js` web checkout) — logged as `Using RevenueCat in Browser Mode`. This opens a web checkout overlay instead of the real StoreKit sheet, and that overlay can itself get stuck/frozen after a simulated purchase — this is a testing artifact of Expo Go, not a bug in app code. Real purchase-flow testing (and the true StoreKit-collision scenario this file describes) requires a dev/EAS build, not Expo Go.
