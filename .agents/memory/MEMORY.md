...
- [Decision-flow log bundle](sagatrail-decision-log-bundle.md) — if camera/story logs arrive but decision_flow is absent, verify the mounted bundle/session before analyzing prompt duplication.
...
- [Sagen-Fallback ohne Startkanton](sagatrail-missing-start-canton.md) — fehlt route.region oder passt kein Katalogkanton, Sagen schweizweit nach Nähe sortieren statt den Picker leer zu lassen.
...
- [POI source provenance](sagatrail-poi-source-provenance.md) — narrative facts need a cited text source; Commons metadata supports image credit, never factual prose.
...
- [SagaTrail wearable integrations](sagatrail-wearable-memory-index.md) — grouped pointers for Apple Watch and Garmin state, payload, localization, battery, UI, and native-build constraints.
