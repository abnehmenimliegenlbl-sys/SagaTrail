...
**How to apply:** Run the normal Drizzle development push through a pseudo-TTY when the runner has no TTY. Select create for genuinely new tables and the no-truncate option for existing tables; if Drizzle proposes dropping legacy columns that the app still exposes, restore those nullable columns in the schema source before pushing. Never use this as a reason to add startup DDL or manually alter production.