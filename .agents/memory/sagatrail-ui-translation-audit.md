...
**How to apply:** Audit dictionary key parity across all nine supported languages, then separately search visible JSX text, accessibility labels, placeholders, and `?? "..."` fallbacks. Make user-facing fields required when every language must provide them; exclude route/POI names, units, technical identifiers, logs, and narrative content that is intentionally sourced elsewhere.