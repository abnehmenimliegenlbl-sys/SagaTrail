// Share-Karte: 35 Walserweg Etappe 12 Chamonas d'Ela – Monstein
// Saga: Die Sennerin vom Piz Err und der Zauberkäse

import { useMemo } from "react";

// ─── Echte Geometrie aus DB (osm-20128211, jeden 25. Punkt) ──────────────────
const GEOMETRY: [number, number][] = [
  [46.6187183, 9.6917101],
  [46.6289320, 9.6908987],
  [46.6419651, 9.6836255],
  [46.6578758, 9.6834031],
  [46.6632081, 9.6839267],
  [46.6646025, 9.6845550],
  [46.6682208, 9.6868720],
  [46.6717026, 9.6876175],
  [46.6772059, 9.6898832],
  [46.6809393, 9.6969507],
  [46.6844965, 9.7116727],
  [46.6885836, 9.7175096],
  [46.6948091, 9.7253096],
  [46.6936735, 9.7333926],
  [46.6946715, 9.7469434],
  [46.6990365, 9.7516004],
  [46.7091419, 9.7639999],
  [46.7107098, 9.7733431],
];

// ─── Routendaten (aus DB) ────────────────────────────────────────────────────
const ROUTE_NAME = "Walserweg Etappe 12\nChamonas d'Ela – Monstein";
const CANTON = "Graubünden";
const DISTANCE_KM = 22;
const ASCENT_M = 1100;
const MAX_ALT_M = 2252;
const SAC_SCALE = "T1";
const DURATION_MIN = 413;
const STEPS = 31200;
const VISITED_PLACES = 3;
const SAGA_TITLE = "Die Sennerin vom Piz Err und der Zauberkäse";
...
function buildRouteMap(geometry: [number, number][], width: number, height: number, padding: number) {
...
// ─── Höhenprofil: Start ~1580m (Chamonas d'Ela), Gipfel ~2252m, Monstein ~1550m
function makeElevProfile() {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    let alt: number;
    if (t < 0.1)       alt = 1580 + t / 0.1 * 80;           // sanfter Einstieg
    else if (t < 0.55) alt = 1660 + ((t - 0.1) / 0.45) * 592; // Hauptanstieg auf 2252m
    else if (t < 0.65) alt = 2252 - ((t - 0.55) / 0.1) * 120; // kurzer Abstieg
    else if (t < 0.75) alt = 2132 + ((t - 0.65) / 0.1) * 60;  // flaches Stück
    else               alt = 2192 - ((t - 0.75) / 0.25) * 642; // Abstieg nach Monstein
    alt += Math.sin(i * 1.5) * 14 + Math.cos(i * 3.1) * 8;
...
function ElevationMiniChart({ width }: { width: number }) {
  ...
  const minAlt = Math.min(...ELEV_PROFILE.map((p) => p.altM));
  const maxAlt = Math.max(...ELEV_PROFILE.map((p) => p.altM));
  const maxDist = ELEV_PROFILE[ELEV_PROFILE.length - 1].distanceKm;
  const toX = (d: number) => (d / maxDist) * width;
  const toY = (a: number) => PAD_TOP + (1 - (a - minAlt) / Math.max(maxAlt - minAlt, 1)) * chartH;
  const pts = ELEV_PROFILE.map((p) => `${toX(p.distanceKm).toFixed(1)},${toY(p.altM).toFixed(1)}`);
  ...
  return (
    <div style={{ position: "relative" }}>
      <svg width={width} height={H} viewBox={`0 0 ${width} ${H}`}>
        ...
        <path d={`M${pts[0]}L${pts.join("L")}L${toX(maxDist).toFixed(1)},${baseY}L0,${baseY}Z`} fill="url(#ef2)" />
      ...
      {route ? (
        <>
          {route.tiles.map((tile) => (
            <img key={tile.key} src={tile.uri} alt="" style={{ position: "absolute", left: tile.left, top: tile.top, width: tile.size, height: tile.size, display: "block" }} />
          ))}
          <svg width={W} height={MAP_H} viewBox={`0 0 ${W} ${MAP_H}`} style={{ position: "absolute", inset: 0 }}>
    ...
    <div style={{ background: "#f4f5f7", minHeight: "100vh", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "24px 0 40px" }}>
      <div style={{ width: CARD_W, background: "#ffffff", boxShadow: "0 4px 24px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.07)" }}>
        ...
        <div style={{ height: 240, position: "relative", overflow: "hidden" }}>
          <img src="/__mockup/images/walserweg12.webp" alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 40%" }} />
