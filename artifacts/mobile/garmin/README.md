...
Before a build, run `pnpm run test:garmin` from `artifacts/mobile`. The first
check validates the manifest product IDs against the deliberately supported
watch list. On Linux, `monkeyc` may still report valid products as invalid when
the desktop device catalog is not installed; that is an SDK-environment
diagnostic, not evidence that the manifest IDs are wrong.
