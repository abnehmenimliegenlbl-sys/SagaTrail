...
export function isLocalTerrainModel(value: unknown): value is LocalTerrainModel {
  ...
  if (
    ...
    !Number.isInteger(model.sectors) ||
    model.sectors < 8 ||
    model.sectors > 72 ||
    typeof model.rings !== "number" ||
    !Number.isInteger(model.rings) ||
    model.rings < 4 ||
    model.rings > 96 ||
    typeof model.fetchedAt !== "number" ||
  ...
  if (
    model.rays.length < Math.max(4, Math.floor(model.sectors / 2)) ||
    model.rays.length > 512
  ) {
    return false;
  }
  const radiusM = model.radiusM as number;
  return model.rays.every(
    (ray) => {
      if (
        ...
        ray.samples.length < 2
      ) {
        return false;
      }
      return ray.samples.every((sample, index) => {
        if (
          !sample ||
          typeof sample.distanceM !== "number" ||
          !Number.isFinite(sample.distanceM) ||
          sample.distanceM < 0 ||
          sample.distanceM > radiusM ||
        ...
        return (
          index === 0 ||
          sample.distanceM > ray.samples[index - 1]!.distanceM
        );
      });
    },
  );
