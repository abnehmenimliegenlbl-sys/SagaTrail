...
exports.distanzZuSegmentKm = distanzZuSegmentKm;
exports.filterByRouteCorridor = filterByRouteCorridor;
exports.fortschrittAufRoute = fortschrittAufRoute;
exports.bearingDeg = bearingDeg;
exports.compassIndex = compassIndex;
exports.decodePolyline6 = decodePolyline6;
exports.bboxAroundGeometry = bboxAroundGeometry;
 ...
 */
function distanzZuSegmentKm(p, a, b) {
 ...
 * die Karte bringen.
 */
function filterByRouteCorridor(items, geometry, maxKm) {
    ...
    return items.filter((item) => {
        ...
        for (let i = 0; i < geometry.length - 1; i++) {
            if (distanzZuSegmentKm(item, { lat: geometry[i][0], lng: geometry[i][1] }, { lat: geometry[i + 1][0], lng: geometry[i + 1][1] }) <= maxKm) {
                return true;
            }
        }
        return false;
 ...
 * Route begonnen wird.
 */
function fortschrittAufRoute(p, geometry) {
    if (!geometry || geometry.length < 2)
        return null;
    let cumKm = 0;
    let bestAlongKm = 0;
    ...
    for (let i = 1; i < geometry.length; i++) {
        const a = { lat: geometry[i - 1][0], lng: geometry[i - 1][1] };
        const b = { lat: geometry[i][0], lng: geometry[i][1] };
 ...
 */
function bboxAroundGeometry(geometry, center, paddingKm = 3) {
