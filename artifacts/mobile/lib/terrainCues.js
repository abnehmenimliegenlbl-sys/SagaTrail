...
exports.calculateProfileAscentM = calculateProfileAscentM;
exports.getSmoothedGradePctAtDistance = getSmoothedGradePctAtDistance;
exports.buildRouteGradeSegments = buildRouteGradeSegments;
exports.buildTerrainSections = buildTerrainSections;
exports.limitTerrainSectionsForSpeech = limitTerrainSectionsForSpeech;
...
function profileAltitude(points, distanceKmValue) {
    return interpolate(points, distanceKmValue);
}
function gradeBand(gradePct) {
    const absoluteGrade = Math.abs(gradePct);
    if (absoluteGrade >= 30 - GRADE_THRESHOLD_EPSILON_PCT)
        return "red";
    if (absoluteGrade >= 20 - GRADE_THRESHOLD_EPSILON_PCT)
        return "orange";
    if (absoluteGrade >= 10 - GRADE_THRESHOLD_EPSILON_PCT)
        return "yellow";
    return "green";
}
/**
 ...
 */
function smoothIsolatedProfileSpikes(points) {
    if (points.length < 3)
        return points;
    return points.map((point, index) => {
        ...
        const isLocalExtremum = (point.altM > previous.altM && point.altM > next.altM) ||
...
function calculateProfileAscentM(points) {
    const valid = [];
...
function pointAtDistance(coords, distances, distanceKmValue) {
    if (distanceKmValue <= 0)
        return coords[0];
    const lastIndex = coords.length - 1;
    if (distanceKmValue >= distances[lastIndex])
        return coords[lastIndex];
    let low = 0;
    let high = lastIndex;
    while (low + 1 < high) {
        const middle = Math.floor((low + high) / 2);
        if (distances[middle] <= distanceKmValue)
            low = middle;
        else
            high = middle;
    }
...
function gradeAtDistance(profile, distanceKmValue, routeLengthKm) {
 ...
 */
function getSmoothedGradePctAtDistance(inputProfile, distanceKmValue) {
    ...
    if (profileLengthKm <= 0)
        return null;
    return gradeAtDistance(smoothIsolatedProfileSpikes(normalizedProfile), Math.max(0, Math.min(profileLengthKm, distanceKmValue)), profileLengthKm);
}
/**
 ...
 */
function buildRouteGradeSegments(geometry, inputProfile) {
    if (!geometry || geometry.length < 2)
        return [];
    const coords = geometry.filter((point) => Array.isArray(point) &&
        point.length >= 2 &&
    ...
    for (let index = 1; index < coords.length; index++) {
        routeDistances.push(routeDistances[index - 1] + distanceKm(coords[index - 1], coords[index]));
    }
    ...
    const flatSegments = () => {
        ...
        const uniqueBreakDistances = breakDistances.filter((distanceKmValue, index) => index === 0 || distanceKmValue - breakDistances[index - 1] > 0.000001);
    ...
    return uniqueBreakDistances.slice(1).map((endDistanceKm, index) => {
        ...
        const gradePct = gradeAtDistance(gradingProfile, (startDistanceKm + endDistanceKm) / 2, routeLengthKm);
        return {
            coordinates: [
...
function interpolate(points, distanceKm) {
    if (distanceKm <= points[0].distanceKm)
        return points[0].altM;
 ...
 */
function buildTerrainSections(input) {
    ...
    for (let index = 1; index < smoothed.length; index++) {
        ...
        bins.push({
            ...
            gradePct,
            direction: gradePct >= ANNOUNCE_GRADE_PCT
                ? "up"
    ...
    for (const bin of bins) {
        ...
        if (run.length > 0 && run[0].direction !== bin.direction)
            flush();
        run.push(bin);
    }
    flush();
 ...
 */
function limitTerrainSectionsForSpeech(input) {
    if (!input || input.length < 2)
        return input ?? [];
    const sorted = [...input].sort((a, b) => a.startKm - b.startKm);
    const selected = [];
    const strength = (section) => (section.isVerySteep ? 1000 : 0) +
