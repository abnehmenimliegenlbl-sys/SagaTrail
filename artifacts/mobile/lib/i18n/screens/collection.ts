import { createUseStrings, StringsDict } from "../createStrings";

export interface CollectionStrings {
  eyebrow: string;
  title: string;
  sagasExperienced: string;
  total: string;
  cantons: string;
  emptyState: string;
  albumTitle: string;
  cantonProgress: (discovered: number, total: number, canton: string) => string;
  emptyCta: string;
  statsTitle: string;
  statsKm: string;
  statsAscent: string;
  statsHikes: string;
  statsDuration: string;
  statsRecordKm: string;
  statsRecordAscent: string;
  statsMonthly: string;
  diaryTitle: string;
  exportGpx: string;
  exportGpxNoData: string;
  exportGpxError: string;
  rankSectionTitle: string;
  ranks: string[];
  pointsLabel: string;
  nextRankProgress: (remaining: number, nextRank: string) => string;
  maxRankReached: string;
}

const COLLECTION_STRINGS: StringsDict<CollectionStrings> = {
  de: {
    eyebrow: "Deine Reise",
    title: "Sammlung",
    sagasExperienced: "Sagen erlebt",
    total: "Insgesamt",
    cantons: "Kantone",
    emptyState: "Noch keine Sage erlebt. Starte deine erste Wanderung, um Funken zu sammeln.",
    albumTitle: "Sagen-Album",
    cantonProgress: (d, t, c) => `${d} von ${t} Sagen in ${c} entdeckt`,
    emptyCta: "Erste Wanderung starten",
    statsTitle: "Wanderstatistik",
    statsKm: "Kilometer",
    statsAscent: "Höhenmeter",
    statsHikes: "Wanderungen",
    statsDuration: "Stunden",
    statsRecordKm: "Längste Tour",
    statsRecordAscent: "Meiste HM",
    statsMonthly: "Aktivität (12 Monate)",
    diaryTitle: "Wander-Tagebuch",
    exportGpx: "GPX exportieren",
    exportGpxNoData: "Für diese Wanderung sind keine Koordinaten gespeichert.",
    exportGpxError: "GPX-Export fehlgeschlagen.",
    rankSectionTitle: "Dein Rang",
    ranks: [
      "Neuling",
      "Wanderfreund",
      "Pfadfinder",
      "Sagenhüter",
      "Bergwanderer",
      "Sagenkenner",
      "Gipfelstürmer",
      "Alpenlegende",
      "Sagenmeister",
      "SagaTrail-Legende",
    ],
    pointsLabel: "Funken",
    nextRankProgress: (remaining, nextRank) => `Noch ${remaining} Funken bis „${nextRank}“`,
    maxRankReached: "Höchster Rang erreicht!",
  },
  gsw: {
    eyebrow: "Dini Reis",
    title: "Sammlig",
    sagasExperienced: "Sage erläbt",
    total: "Insgsamt",
    cantons: "Kantön",
    emptyState: "No kei Sag erläbt. Start dini erschti Wanderig, zum Funke sammle.",
    albumTitle: "Sage-Album",
    cantonProgress: (d, t, c) => `${d} vo ${t} Sage in ${c} entdeckt`,
    emptyCta: "Erschti Wanderig starte",
    statsTitle: "Wanderstatistik",
    statsKm: "Kilometer",
    statsAscent: "Höhemeter",
    statsHikes: "Wanderige",
    statsDuration: "Stunde",
    statsRecordKm: "Lengscht Tour",
    statsRecordAscent: "Meischti HM",
    statsMonthly: "Aktivität (12 Mönet)",
    diaryTitle: "Wander-Tagebuech",
    exportGpx: "GPX exportiere",
    exportGpxNoData: "Für die Wanderig sind kei Koordinate gspichert.",
    exportGpxError: "GPX-Export fehlgschlage.",
    rankSectionTitle: "Dyn Rang",
    ranks: [
      "Nöilig",
      "Wanderfründ",
      "Pfadfinder",
      "Sagehüeter",
      "Bärgwanderer",
      "Sagechenner",
      "Gipfelstürmer",
      "Alpelegändi",
      "Sagemeister",
      "SagaTrail-Legändi",
    ],
    pointsLabel: "Funke",
    nextRankProgress: (remaining, nextRank) => `No ${remaining} Funke bis „${nextRank}“`,
    maxRankReached: "Höchschte Rang erreicht!",
  },
  en: {
    eyebrow: "Your Journey",
    title: "Collection",
    sagasExperienced: "Sagas experienced",
    total: "Total",
    cantons: "Cantons",
    emptyState: "No sagas experienced yet. Start your first hike to collect sparks.",
    albumTitle: "Saga Album",
    cantonProgress: (d, t, c) => `${d} of ${t} sagas discovered in ${c}`,
    emptyCta: "Start your first hike",
    statsTitle: "Hiking stats",
    statsKm: "Kilometres",
    statsAscent: "Metres of ascent",
    statsHikes: "Hikes",
    statsDuration: "Hours",
    statsRecordKm: "Longest hike",
    statsRecordAscent: "Most ascent",
    statsMonthly: "Activity (12 months)",
    diaryTitle: "Hiking diary",
    exportGpx: "Export GPX",
    exportGpxNoData: "No coordinates recorded for this hike.",
    exportGpxError: "GPX export failed.",
    rankSectionTitle: "Your rank",
    ranks: [
      "Newcomer",
      "Trail Friend",
      "Pathfinder",
      "Saga Keeper",
      "Mountain Hiker",
      "Saga Connoisseur",
      "Summit Chaser",
      "Alpine Legend",
      "Saga Master",
      "SagaTrail Legend",
    ],
    pointsLabel: "Sparks",
    nextRankProgress: (remaining, nextRank) => `${remaining} sparks to reach "${nextRank}"`,
    maxRankReached: "Highest rank reached!",
  },
  fr: {
    eyebrow: "Ton voyage",
    title: "Collection",
    sagasExperienced: "Légendes vécues",
    total: "Total",
    cantons: "Cantons",
    emptyState: "Aucune légende vécue pour l'instant. Commence ta première randonnée pour collecter des étincelles.",
    albumTitle: "Album des légendes",
    cantonProgress: (d, t, c) => `${d} légende(s) sur ${t} découverte(s) en ${c}`,
    emptyCta: "Commencer ta première randonnée",
    statsTitle: "Statistiques de randonnée",
    statsKm: "Kilomètres",
    statsAscent: "Dénivelé (m)",
    statsHikes: "Randonnées",
    statsDuration: "Heures",
    statsRecordKm: "Rando la plus longue",
    statsRecordAscent: "Dénivelé max.",
    statsMonthly: "Activité (12 mois)",
    diaryTitle: "Journal de randonnée",
    exportGpx: "Exporter GPX",
    exportGpxNoData: "Aucune coordonnée enregistrée pour cette randonnée.",
    exportGpxError: "L'export GPX a échoué.",
    rankSectionTitle: "Ton rang",
    ranks: [
      "Débutant",
      "Ami des sentiers",
      "Éclaireur",
      "Gardien des légendes",
      "Randonneur de montagne",
      "Connaisseur de légendes",
      "Chasseur de sommets",
      "Légende alpine",
      "Maître des légendes",
      "Légende SagaTrail",
    ],
    pointsLabel: "Étincelles",
    nextRankProgress: (remaining, nextRank) => `Encore ${remaining} étincelles pour « ${nextRank} »`,
    maxRankReached: "Rang maximal atteint !",
  },
  it: {
    eyebrow: "Il tuo viaggio",
    title: "Collezione",
    sagasExperienced: "Leggende vissute",
    total: "Totale",
    cantons: "Cantoni",
    emptyState: "Ancora nessuna leggenda vissuta. Inizia la tua prima escursione per raccogliere scintille.",
    albumTitle: "Album delle leggende",
    cantonProgress: (d, t, c) => `${d} leggende su ${t} scoperte in ${c}`,
    emptyCta: "Inizia la tua prima escursione",
    statsTitle: "Statistiche escursioni",
    statsKm: "Chilometri",
    statsAscent: "Dislivello (m)",
    statsHikes: "Escursioni",
    statsDuration: "Ore",
    statsRecordKm: "Escursione più lunga",
    statsRecordAscent: "Massimo dislivello",
    statsMonthly: "Attività (12 mesi)",
    diaryTitle: "Diario delle escursioni",
    exportGpx: "Esporta GPX",
    exportGpxNoData: "Nessuna coordinata registrata per questa escursione.",
    exportGpxError: "Esportazione GPX fallita.",
    rankSectionTitle: "Il tuo rango",
    ranks: [
      "Principiante",
      "Amico dei sentieri",
      "Esploratore",
      "Custode delle leggende",
      "Escursionista di montagna",
      "Conoscitore di leggende",
      "Cacciatore di vette",
      "Leggenda alpina",
      "Maestro delle leggende",
      "Leggenda SagaTrail",
    ],
    pointsLabel: "Scintille",
    nextRankProgress: (remaining, nextRank) => `Ancora ${remaining} scintille per "${nextRank}"`,
    maxRankReached: "Rango massimo raggiunto!",
  },
  es: {
    eyebrow: "Tu viaje",
    title: "Colección",
    sagasExperienced: "Leyendas vividas",
    total: "Total",
    cantons: "Cantones",
    emptyState: "Aún no has vivido ninguna leyenda. Comienza tu primera caminata para recolectar chispas.",
    albumTitle: "Álbum de leyendas",
    cantonProgress: (d, t, c) => `${d} de ${t} leyendas descubiertas en ${c}`,
    emptyCta: "Comienza tu primera caminata",
    statsTitle: "Estadísticas de caminatas",
    statsKm: "Kilómetros",
    statsAscent: "Desnivel (m)",
    statsHikes: "Caminatas",
    statsDuration: "Horas",
    statsRecordKm: "Caminata más larga",
    statsRecordAscent: "Máximo desnivel",
    statsMonthly: "Actividad (12 meses)",
    diaryTitle: "Diario de caminatas",
    exportGpx: "Exportar GPX",
    exportGpxNoData: "No hay coordenadas registradas para esta caminata.",
    exportGpxError: "Error al exportar GPX.",
    rankSectionTitle: "Tu rango",
    ranks: [
      "Principiante",
      "Amigo del sendero",
      "Explorador",
      "Guardián de leyendas",
      "Caminante de montaña",
      "Conocedor de leyendas",
      "Cazador de cumbres",
      "Leyenda alpina",
      "Maestro de leyendas",
      "Leyenda SagaTrail",
    ],
    pointsLabel: "Chispas",
    nextRankProgress: (remaining, nextRank) => `Aún faltan ${remaining} chispas para "${nextRank}"`,
    maxRankReached: "¡Rango máximo alcanzado!",
  },
  pt: {
    eyebrow: "Sua jornada",
    title: "Coleção",
    sagasExperienced: "Lendas vividas",
    total: "Total",
    cantons: "Cantões",
    emptyState: "Nenhuma lenda vivida ainda. Comece sua primeira caminhada para coletar faíscas.",
    albumTitle: "Álbum de lendas",
    cantonProgress: (d, t, c) => `${d} de ${t} lendas descobertas em ${c}`,
    emptyCta: "Comece sua primeira caminhada",
    statsTitle: "Estatísticas de caminhadas",
    statsKm: "Quilômetros",
    statsAscent: "Desnível (m)",
    statsHikes: "Caminhadas",
    statsDuration: "Horas",
    statsRecordKm: "Maior caminhada",
    statsRecordAscent: "Maior desnível",
    statsMonthly: "Atividade (12 meses)",
    diaryTitle: "Diário de caminhadas",
    exportGpx: "Exportar GPX",
    exportGpxNoData: "Nenhuma coordenada registrada para esta caminhada.",
    exportGpxError: "A exportação GPX falhou.",
    rankSectionTitle: "Seu ranque",
    ranks: [
      "Novato",
      "Amigo da trilha",
      "Batedor",
      "Guardião de lendas",
      "Caminhante de montanha",
      "Conhecedor de lendas",
      "Caçador de cumes",
      "Lenda alpina",
      "Mestre de lendas",
      "Lenda SagaTrail",
    ],
    pointsLabel: "Faíscas",
    nextRankProgress: (remaining, nextRank) => `Faltam ${remaining} faíscas para "${nextRank}"`,
    maxRankReached: "Ranque máximo alcançado!",
  },
  zh: {
    eyebrow: "你的旅程",
    title: "藏品",
    sagasExperienced: "经历过的传说",
    total: "总计",
    cantons: "联邦州",
    emptyState: "尚未经历任何传说。开始你的第一次徒步，收集火花吧。",
    albumTitle: "传说图鉴",
    cantonProgress: (d, t, c) => `${c}：已发现 ${d}/${t} 个传说`,
    emptyCta: "开始你的第一次徒步",
    statsTitle: "徒步统计",
    statsKm: "公里",
    statsAscent: "爬升（米）",
    statsHikes: "徒步次数",
    statsDuration: "小时",
    statsRecordKm: "最长徒步",
    statsRecordAscent: "最大爬升",
    statsMonthly: "活跃度（12个月）",
    diaryTitle: "徒步日记",
    exportGpx: "导出 GPX",
    exportGpxNoData: "此次徒步未记录坐标。",
    exportGpxError: "GPX 导出失败。",
    rankSectionTitle: "你的等级",
    ranks: [
      "新手",
      "步道之友",
      "探路者",
      "传说守护者",
      "登山者",
      "传说达人",
      "峰顶追逐者",
      "阿尔卑斯传奇",
      "传说大师",
      "SagaTrail传奇",
    ],
    pointsLabel: "火花",
    nextRankProgress: (remaining, nextRank) => `还差 ${remaining} 火花即可晋升为"${nextRank}"`,
    maxRankReached: "已达到最高等级！",
  },
  ru: {
    eyebrow: "Твоё путешествие",
    title: "Коллекция",
    sagasExperienced: "Легенды пройдены",
    total: "Всего",
    cantons: "Кантоны",
    emptyState: "Пока ни одной легенды не пройдено. Начни свой первый поход, чтобы собрать искры.",
    albumTitle: "Альбом легенд",
    cantonProgress: (d, t, c) => `Открыто ${d} из ${t} легенд в ${c}`,
    emptyCta: "Начать первый поход",
    statsTitle: "Статистика походов",
    statsKm: "Километры",
    statsAscent: "Набор высоты (м)",
    statsHikes: "Походы",
    statsDuration: "Часы",
    statsRecordKm: "Самый длинный поход",
    statsRecordAscent: "Макс. набор высоты",
    statsMonthly: "Активность (12 месяцев)",
    diaryTitle: "Дневник походов",
    exportGpx: "Экспорт GPX",
    exportGpxNoData: "Координаты для этого похода не записаны.",
    exportGpxError: "Ошибка экспорта GPX.",
    rankSectionTitle: "Твой ранг",
    ranks: [
      "Новичок",
      "Друг тропы",
      "Следопыт",
      "Хранитель легенд",
      "Горный путник",
      "Знаток легенд",
      "Покоритель вершин",
      "Альпийская легенда",
      "Мастер легенд",
      "Легенда SagaTrail",
    ],
    pointsLabel: "Искры",
    nextRankProgress: (remaining, nextRank) => `Ещё ${remaining} искр до ранга «${nextRank}»`,
    maxRankReached: "Достигнут максимальный ранг!",
  },
};

export const useCollectionStrings = createUseStrings(COLLECTION_STRINGS);
