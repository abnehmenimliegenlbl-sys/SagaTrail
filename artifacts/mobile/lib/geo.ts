...
const BUS_AND_TRAM_STOP_KINDS = new Set([
  "highway=bus_stop",
  "railway=tram_stop",
]);

/** Bus- und Tramhaltestellen sind nur an den beiden Routenenden relevante POIs. */
export function isBusOrTramStopPoi(item: { kind?: string | null }): boolean {
...
export function distanceToRouteEndpointKm(
  ...
  const first = { lat: geometry[0]![0]!, lng: geometry[0]![1]! };
  const lastPoint = geometry[geometry.length - 1]!;
  const last = { lat: lastPoint[0]!, lng: lastPoint[1]! };
  return Math.min(haversineKm(point, first), haversineKm(point, last));
}

/**
 * Entfernt Bus-/Tramhaltestellen aus dem allgemeinen POI-Korridor, wenn sie
 * nicht am Start oder Ziel liegen. Andere POI-Arten bleiben unverändert.
 */
export function filterBusAndTramStopsToRouteEndpoints<
  T extends LatLng & { kind?: string | null },
>(
  items: readonly T[],
  geometry: number[][] | null | undefined,
  maxEndpointKm = 0.5,
  ...
  return items.filter(
    (item) =>
      !isBusOrTramStopPoi(item) ||
      distanceToRouteEndpointKm(item, geometry) <= maxEndpointKm,
  );
}
