...
export default function LiveHike() {
  ...
  const hasFreshGps = isFreshGpsFix({
    permissionGranted: locState === "granted",
    hasPosition: livePos !== null,
    lastFixAtMs: lastLocationAtRef.current,
    nowMs: locationNow,
  });
  ...
  const walkToStart = useMemo(() => {
    if (!hasFreshGps || !livePos || !navigationGeometry || navigationGeometry.length < 2)
      return null;
    const start: LatLng = {
      lat: navigationGeometry[0][0],
      lng: navigationGeometry[0][1],
  ...
  return (
    <Background>
      ...
      >
        ...
        {/* Story-Bereich */}
        {nowPlayingVisible && nowPlaying && !storyTileOpen && (
          <Animated.View
            entering={FadeInUp}
            style={[
              styles.audioMiniPlayer,
              {
          ...
          >
            <View style={styles.audioMiniMeta}>
              <Feather name="volume-2" size={16} color={colors.accent} />
              <View style={{ flex: 1 }}>
                <Text style={[styles.audioMiniLabel, { color: colors.accent }]}>
                  {nowPlaying.label}
                </Text>
                <Text
                  style={[styles.audioMiniTitle, { color: colors.foreground }]}
                  numberOfLines={1}
                >
            ...
            <Pressable
              onPress={() => void cancelNarration()}
              accessibilityRole="button"
              accessibilityLabel={t.pause}
              style={[styles.audioMiniButton, { borderColor: colors.glassBorder }]}
            >
