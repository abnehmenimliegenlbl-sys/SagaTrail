...
type ThemeListItem =
  | { type: "canton"; key: string; canton: string }
  | { type: "route"; key: string; route: HikingRoute; canton: string; index: number };
...
export default function ThemenweltRoute() {
  ...
  useEffect(() => {
    ...
    // Themenwelt-Antworten dürfen nicht als leere 304-Antwort im Client
    // landen: Die Liste ist dynamisch und wird vollständig als JSON benötigt.
    getThemeRoutes(theme, { cache: "no-store" })
      .then((routes) => {
        if (cancelled) return;
  ...
  const listItems = useMemo<ThemeListItem[]>(() => {
    const items: ThemeListItem[] = [];
    let routeIndex = 0;
    for (const [canton, cantonRoutes] of groupedMatches) {
      items.push({ type: "canton", key: `canton:${canton}`, canton });
      for (const { route } of cantonRoutes) {
        items.push({
          type: "route",
          key: `route:${route.id}`,
          route,
          canton,
          index: Math.min(routeIndex++, 12),
        });
      }
    }
    return items;
