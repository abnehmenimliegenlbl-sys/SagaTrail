...
<head>
  ...
  <style>
    ...
    .safety-cluster { min-width: 26px; height: 26px; padding: 0 4px; border-radius: 50%; background: #B21F2D; border: 1.5px solid #fffaf0; box-shadow: 0 0 0 2.25px rgba(178,31,45,.25), 0 1.5px 6px rgba(0,0,0,.35); box-sizing: border-box; color: #fff; font: 800 9px -apple-system,system-ui,sans-serif; text-align: center; line-height: 23px; opacity: .75; }
  ...
  (function () {
    ...
    function renderPoiClusters() {
      ...
      Object.keys(buckets).forEach(function (key) {
        ...
        var cluster = L.marker([lat, lng], {
          icon: icon("poi-cluster", String(group.length), [27,27]),
          zIndexOffset: 500
        }).addTo(map);
    ...
    function renderSafetyClusters() {
      ...
      Object.keys(buckets).forEach(function (key) {
        ...
        var lng = group.reduce(function (sum, p) { return sum + p.lng; }, 0) / group.length;
        var cluster = L.marker([lat, lng], {
          icon: icon("safety-cluster", group.length > 99 ? "99+" : String(group.length), [32,32]),
          zIndexOffset: 700
        }).addTo(map);
    ...
    if (drawingMode) {
      ...
      drawContainer.style.userSelect = "none";
       ...
       function sampleAlongPath(points, maxPoints) {
         ...
         var cursor = 1;
         for (var targetIndex = 1; targetIndex < maxPoints - 1; targetIndex++) {
           var targetDistance = total * targetIndex / (maxPoints - 1);
           while (cursor < points.length - 1 && cumulative[cursor] < targetDistance) {
             cursor++;
           }
           var before = points[cursor - 1];
           var after = points[cursor];
           var beforeDistance = cumulative[cursor - 1];
           var segmentDistance = cumulative[cursor] - beforeDistance;
           var ratio = segmentDistance > 0
      ...
      function finishDrawing(event) {
        ...
        if (drawnPoints.length < 2) return;
         var sampled = sampleAlongPath(drawnPoints, 100);
