...
import {
  startEnrichAllIfNeeded,
  scheduleNightlyRestitch,
  scheduleNightlyDifficultyAudit,
} from "./routes/admin";
...
const server = app.listen(port, async (err) => {
  ...
  // Danach den offenen SAC-Bestand gegen aktuelle OSM-Tags prüfen (read-only).
  scheduleNightlyDifficultyAudit(logger);
});
