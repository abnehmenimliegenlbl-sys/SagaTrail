...
let difficultyAuditLaeuft = false;
...
/**
 * Prüft täglich den offenen SchweizMobil-Routenbestand gegen aktuelle
 * OSM-sac_scale-Tags. Der Lauf bleibt absichtlich schreibgeschützt: Änderungen
 * werden weiterhin nur über den geschützten Admin-Audit mit dryRun=false
 * übernommen.
 */
export function scheduleNightlyDifficultyAudit(log: Logger): void {
  function msUntilNextRun(): number {
    const now = new Date();
    const next = new Date(now);
  ...
  function scheduleNext(): void {
    const ms = msUntilNextRun();
    log.info(
      { inMinutes: Math.round(ms / 60_000) },
      "difficulty-audit: nächster schreibgeschützter Lauf geplant",
    );
    setTimeout(async () => {
      if (!difficultyAuditLaeuft) {
        difficultyAuditLaeuft = true;
        try {
          const result = await auditSchweizMobilDifficulties(log, { dryRun: true });
          log.info(
            {
              scanned: result.scanned,
              exactSac: result.exactSac,
              officialCategories: result.officialCategories,
              conflicts: result.conflicts,
              unknown: result.unknown,
            },
            "difficulty-audit: schreibgeschützter Lauf abgeschlossen",
          );
        } catch (err) {
          log.warn({ err }, "difficulty-audit: Nacht-Lauf fehlgeschlagen");
        } finally {
          difficultyAuditLaeuft = false;
        }
      }
      scheduleNext();
    }, ms);
