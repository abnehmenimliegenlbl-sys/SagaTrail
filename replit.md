...
- GPX import (`POST /routes/gpx`): regex-based trkpt/rtept parser (namespace-tolerant), Switzerland bbox check (>=70% of points), then the SAME enrichment as custom routes via shared `buildRouteFromPoints` in `customRoute.ts`; global express.json limit raised to 25mb for GPX payloads; mobile reads the file via expo-document-picker + expo-file-system/legacy (web: fetch(uri)).
