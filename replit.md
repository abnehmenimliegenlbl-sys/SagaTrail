...
- Dynamic per-canton catalog: all 26 cantons are selectable. Opening a canton fetches REAL OpenStreetMap hiking routes (Overpass, enriched with swisstopo ascent; missing SAC difficulty shows "unbekannt") via `loadCantonRoutes`, falling back to the curated seed routes offline. Dynamic routes live in both a ref (async loaders) and state (re-render). Server `external_routes` TTL refresh upserts with `excluded.*` values.
