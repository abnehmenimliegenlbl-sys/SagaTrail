...
- Nearest-saga matching (routes do NOT own a specific saga): each dynamic OSM route resolves to the NEAREST curated saga. `routeService.getRouteSaga` uses `findNearestCuratedSaga`. Mobile `lib/sagaMatch.ts` `nearestSaga` (canton-first haversine, `EXAKT_RADIUS_M = 3500`) drives `getSagaForRoute`/`ensureRouteSaga`; `sagaLokalisierung` marks a route assignment `exakt` only when same canton AND <= 3.5 km, else the UI shows "nicht exakt lokalisierbar". This route-assignment certainty is separate from a saga's intrinsic `koordinatenSicherheit`.
- Dynamic per-canton catalog: all 26 cantons are selectable. Opening a canton fetches REAL OpenStreetMap hiking routes (Overpass, enriched with swisstopo ascent) via `loadCantonRoutes`; there is no offline fallback. The Overpass candidate cap is 30 (`fetchCantonHikingRoutes` `limit`); phase 1 discovers named `route=hiking` relations, phase 2 stitches geometry and drops relations whose geometry cannot be stitched (`points.length < 2`), so the returned count is typically lower than the discovered count. Dynamic routes live in both a ref (async loaders) and state (re-render). Server `external_routes` TTL refresh upserts with `excluded.*` values.
...
- Per-hike offline download (`contexts/DownloadContext.tsx`): downloads the story (server `createStory`, else local `generateStory`) plus a bounded corridor of swisstopo tiles (`lib/offlineTiles.ts`, native only). Live hike resolves stories offline-first via `resolveStory` (local -> server -> local `generateStory`) and passes local tiles to the map. Note: a downloaded route object is what makes a hike replayable offline — there is no route seed to fall back on.
