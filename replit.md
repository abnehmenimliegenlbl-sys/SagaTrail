...
- Dynamic per-canton catalog: all 26 cantons are selectable. Opening a canton fetches REAL OpenStreetMap hiking routes (Overpass, enriched with swisstopo ascent; missing SAC difficulty shows "unbekannt") via `loadCantonRoutes`, falling back to the 30 curated seed routes offline. A location-anchored AI saga is generated lazily per route (`ensureRouteSaga`, saga id == route id, 1:1) only when a route/saga screen opens; first generation is slow (~10s), then cached in Postgres and AsyncStorage (`sagatrail:dynamicCache`). Dynamic routes/sagas live in both a ref (async loaders) and state (re-render). `ensureRouteSaga` dedupes in-flight requests so the route and saga screens don't double-generate. Server `external_routes` TTL refresh upserts with `excluded.*` values.
